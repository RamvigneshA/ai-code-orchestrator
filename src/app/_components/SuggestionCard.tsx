...
export function SuggestionCard({ orchestration, applyChanges, discardChanges }: SuggestionCardProps) {
  ...
  const hasActions = orchestration.actions && orchestration.actions.length > 0;

  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* AI Explanation / Chat Response */}
      <div className="space-y-2">
        <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-tight">AI Response</p>
        <div className="text-[11px] text-slate-300 leading-relaxed font-sans bg-indigo-500/5 p-4 rounded-xl border border-indigo-500/10 shadow-inner">
          {orchestration.explanation || "No explanation provided."}
        </div>
      </div>

      {/* Suggested File Changes (Only show if there are actions) */}
      {hasActions && (
        <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl space-y-4">
          <div className="space-y-1.5">
            <p className="text-[9px] font-bold text-emerald-500 uppercase tracking-wider">Suggested File Changes</p>
          ...
          <div className="flex gap-2 pt-1">
            <button 
              ...
              className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold rounded-lg transition-all shadow-lg shadow-emerald-500/10 active:scale-[0.98]"
            >
              Apply All
            </button>
            <button 
      ...
      {/* Reasoning Details (Optional) */}
      {orchestration.reasoning_details && (
        <div className="space-y-2">
          <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-tight">System Reasoning</p>
